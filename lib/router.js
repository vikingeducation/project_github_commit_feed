...
var _extractPostData = (req, callback) => {
   var body = '';
   req.on('data', (data) => {
      body += data;
   });
   req.on('end', () => {
      if (body !== undefined) {
         ...
         var obj = {};
         obj.name = body.pusher.name;
         obj.repo = body.repository.name;
         callback(obj);
      }
...
Router.routes.post['/github/webhooks'] = (req, res) => {
   res.writeHead(200, __headers);
   _extractPostData(req, (result) => {
      if (result !== undefined) {
         var commits = githubRepoCommits.githubRepoCommits.getCommits(result.name, result.repo);
         if (commits) {
            console.log('Webhook data saved.');
            res.end('200 OK');
         } else {
            console.log('Webhook data missing.');
            res.end('404 ERROR');
         }
      }
   });
