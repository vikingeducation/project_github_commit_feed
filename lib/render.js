...
function render(filename, content) {
  return new Promise(resolve => {
    fs.readFile("./views/" + filename, "utf8", (err, data) => {
      ...
      if (content.length > 0) {
        const filterElement =
          fs.readFileSync("./views/elements/filter.html", "utf8");
        if (filterElement) {
      ...
      const rowTemplate =
        fs.readFileSync("./views/elements/row.html", "utf8");
      if (rowTemplate) {
...
function renderError(error) {
  return new Promise(resolve => {
    fs.readFile("./views/index.html", "utf8", (err, data) => {
      if (err) throw err;
      resolve(data.replace(new RegExp("{{content}}", "gi"), "<h2 class=\"error\">" + error + "</h2>"));
    });
  });
}
